...
void bytecode_switch(char opcode) {
    ...
    case 0x03:
        ...
        pa[0] = next();
        pi = *((int *) pa);
        dbg("\e[36m%d (%02x:0x%02x)\e[0m",pi ,type ,pi);
    ...
    case 0x00:
        ...
        // operace s pameti
    case 0x0c:
        dbg("\e[36m-- alloc\e[0m");
        pa[3] = next();
        pa[2] = next();
        pa[1] = next();
        pa[0] = next();
        pi = *((int *) pa);
        dbg("\e[36m %d + 1 slots (%d bytes)\e[0m", pi, (pi + 1) * sizeof(vm_val));
        alloc(create_integer(pi));
        break;
    case 0x1c:
        dbg("\e[36m-- clalloc\e[0m");
        clalloc();
        break;
    case 0x0d:
        dbg("\e[36m-- ist \e[0m");
...
void run_main() {
    ...
    // zavolej main
    dbg(" Header 4B: \e[33m-- Main found on %d (0x%02x)\e[0m\n", pi, pi);
